...
| BUG-002 | CI publish workflow fails `ENEEDAUTH` — npm auto-publish broken | P1 | resolved | ad-hoc | Converted `publish.yml` to **OIDC trusted publishing** (tokenless). v0.6.6 first attempt then failed `E404` — setup-node@v4/Node 22 shipped npm 10.9 (< 11.5.1) which ignores OIDC and used setup-node's empty-token `.npmrc`. Corrected (branch `fix/oidc-publish-config`) to the official config: setup-node@v6 + Node 24 (npm ≥ 11.5.1), `package-manager-cache: false`. Trusted publisher registered on npmjs.com. **Resolved in v0.6.8** (2026-07-05) — `registry-url` removed, tokenless OIDC publish with auto-provenance succeeded. Row was left `in-progress` after the fix shipped; corrected 2026-08-12. |
