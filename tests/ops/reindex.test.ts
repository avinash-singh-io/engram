...
describe('reindex reports what the walker found', () => {
  ...
  it('an empty vault reindexes without error', async () => {
    ...
    // The derived files, exactly. Rendered skills and commands are asserted
    // separately because their number is a property of the registry, and pinning
    // it here would make adding an operation fail a test about reindexing an
    // empty vault.
    expect(
      written.filter((p) => !isSkillPath(p) && !isCommandPath(p) && p !== '/.gitignore'),
    ).toEqual([
      '/index.md',
      '/views/superseded.md',
      '/views/recent.md',
